...
export const signup = async (req, res) => {
    ...
    try {
        if (password.length < 6) {
            return res.status(400).json({ messege: "password must be atleast 6 character" })
        }

        const user = await User.findOne({ email })
        if (user) return res.status(400).json({ messege: "Email aleady exists" })
...
export const login = async (req, res) => {
    ...
    try {
        const user = await User.findOne({ email })
        if (!user) {
            return req.status(400).json({ messege: "Invalid credentials" })
        }
...
export const updateProfile = async (req, res) => {
    try {
        ...
        console.log("error in update profile", error.messege)
        res.status(500).json({ messege: "Internal server error" })
    }
}
...
export const checkAuth = (req, res) => {
    try {
        ...
        console.log("Error in checkAuth controller", error.messege)
        res.status(500).json({ messege: "Internal server error" })
    }
}
